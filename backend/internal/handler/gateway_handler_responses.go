...
func (h *GatewayHandler) Responses(c *gin.Context) {
	...
	releaseStreamReservation := func(reason string) {
		...
		if err := h.executeUsageRecordTask(func(ctx context.Context) error {
			...
			reqLog.Error("gateway.responses.streaming_release_failed",
	...
	for {
		...
		if streamingBillingV2 && !streamReservationPublished {
			if err := h.executeUsageRecordTask(func(ctx context.Context) error {
				...
				reqLog.Error("gateway.responses.streaming_reserve_failed",
					zap.Int64("account_id", account.ID),
					zap.String("request_id", streamReservationID),
					zap.Error(err),
				)
				h.responsesErrorResponse(c, http.StatusServiceUnavailable, "billing_unavailable", "Billing temporarily unavailable")
				return
			}
			...
			if errors.As(err, &failoverErr) {
				...
				if c.Writer.Size() != writerSizeBeforeForward {
					...
					if commitErr := commitBufferedResponseOrWriteError(c, responseCapture, func() {
						h.responsesErrorResponse(c, http.StatusServiceUnavailable, "server_error", "Response too large")
					}); commitErr != nil {
						reqLog.Error("gateway.responses.commit_buffered_response_failed", zap.Error(commitErr))
					}
					return
				}
		...
		if streamingBillingV2 {
			...
			if err != nil {
				reqLog.Error("gateway.responses.record_stream_finalize_failed",
					zap.Int64("account_id", account.ID),
					zap.String("request_id", streamReservationID),
					zap.Error(err),
				)
			...
			if terminalCapture != nil {
				if commitErr := terminalCapture.CommitTerminal(c); commitErr != nil {
					reqLog.Error("gateway.responses.commit_terminal_stream_failed", zap.Error(commitErr))
				}
			}
			return
		}
