...
import (
	...
	"github.com/wchen99998/robust2api/internal/pkg/oauth"
...
// OIDCOAuthStart starts the generic OIDC OAuth login flow.
func (h *AuthHandler) OIDCOAuthStart(c *gin.Context) {
	...
	authURL, err := buildOIDCAuthorizeURL(cfg, state, nonce, codeChallenge, strings.TrimSpace(cfg.RedirectURL))
	if err != nil {
...
// OIDCOAuthCallback validates the OIDC callback, signs the user in, then redirects to the frontend.
func (h *AuthHandler) OIDCOAuthCallback(c *gin.Context) {
	...
	tokenResp, err := oidcExchangeCode(c.Request.Context(), cfg, code, strings.TrimSpace(cfg.RedirectURL), codeVerifier)
	if err != nil {
		description := ""
	...
	tokenPair, _, err := h.authService.LoginOrRegisterOAuthWithRegistrationEmail(c.Request.Context(), email, registrationEmail, username, "")
	if err != nil {
		if errors.Is(err, service.ErrOAuthInvitationRequired) {
			pendingToken, tokenErr := h.authService.CreatePendingOAuthTokenWithRegistrationEmail(email, registrationEmail, username)
			if tokenErr != nil {
...
func (h *AuthHandler) CompleteOIDCOAuthRegistration(c *gin.Context) {
	...
	email, registrationEmail, username, err := h.authService.VerifyPendingOAuthToken(req.PendingOAuthToken)
	if err != nil {
		c.JSON(http.StatusUnauthorized, gin.H{"error": "INVALID_TOKEN", "message": "invalid or expired registration token"})
		return
	}

	tokenPair, _, err := h.authService.LoginOrRegisterOAuthWithRegistrationEmail(c.Request.Context(), email, registrationEmail, username, req.InvitationCode)
	if err != nil {
		response.ErrorFrom(c, err)
		return
	}
