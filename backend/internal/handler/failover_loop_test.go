...
const testMaxSameAccountRetries = 3
...
func TestNewFailoverState(t *testing.T) {
	...
	t.Run("无绑定会话", func(t *testing.T) {
		fs := runtimefailover.NewState(3, false)
		require.Equal(t, 3, fs.MaxSwitches)
	})

	t.Run("零最大切换次数", func(t *testing.T) {
		fs := runtimefailover.NewState(0, false)
		require.Equal(t, 0, fs.MaxSwitches)
	})
}

// ---------------------------------------------------------------------------
// runtimefailover.SleepWithContext 测试
// ---------------------------------------------------------------------------

func TestSleepWithContext(t *testing.T) {
	t.Run("零时长立即返回true", func(t *testing.T) {
		start := time.Now()
		ok := runtimefailover.SleepWithContext(context.Background(), 0)
		require.True(t, ok)
	...
	t.Run("负时长立即返回true", func(t *testing.T) {
		start := time.Now()
		ok := runtimefailover.SleepWithContext(context.Background(), -1*time.Second)
		require.True(t, ok)
		require.Less(t, time.Since(start), 50*time.Millisecond)
	})
	...
	t.Run("正常等待后返回true", func(t *testing.T) {
		start := time.Now()
		ok := runtimefailover.SleepWithContext(context.Background(), 50*time.Millisecond)
	...
	t.Run("已取消context立即返回false", func(t *testing.T) {
		...
		start := time.Now()
		ok := runtimefailover.SleepWithContext(ctx, 5*time.Second)
		require.False(t, ok)
		require.Less(t, time.Since(start), 50*time.Millisecond)
	})
...
func TestHandleFailoverError_BasicSwitch(t *testing.T) {
	t.Run("非重试错误_非Antigravity_直接切换", func(t *testing.T) {
		...
		require.Equal(t, runtimefailover.Continue, action)
	...
	t.Run("连续切换直到耗尽", func(t *testing.T) {
		mock := &mockTempUnscheduler{}
		fs := runtimefailover.NewState(2, false)
		...
		require.Equal(t, runtimefailover.Exhausted, action)
...
func TestHandleFailoverError_CacheBilling(t *testing.T) {
	t.Run("hasBoundSession为true时设置ForceCacheBilling", func(t *testing.T) {
		mock := &mockTempUnscheduler{}
		fs := runtimefailover.NewState(3, true) // hasBoundSession=true
		err := newTestFailoverErr(500, false, false)
...
func TestHandleFailoverError_SameAccountRetry(t *testing.T) {
	t.Run("第一次重试返回runtimefailover.Continue", func(t *testing.T) {
		mock := &mockTempUnscheduler{}
		fs := runtimefailover.NewState(3, false)
	...
	t.Run("达到最大重试次数前均返回runtimefailover.Continue", func(t *testing.T) {
		mock := &mockTempUnscheduler{}
		fs := runtimefailover.NewState(3, false)
		err := newTestFailoverErr(400, true, false)

		for i := 1; i <= testMaxSameAccountRetries; i++ {
			action := fs.HandleFailoverError(context.Background(), mock, 100, "openai", err)
			require.Equal(t, runtimefailover.Continue, action)
	...
	t.Run("超过最大重试次数后触发TempUnschedule并切换", func(t *testing.T) {
		...
		for i := 0; i < testMaxSameAccountRetries; i++ {
			fs.HandleFailoverError(context.Background(), mock, 100, "openai", err)
		}
		require.Equal(t, testMaxSameAccountRetries, fs.SameAccountRetryCount[100])

		// 第 testMaxSameAccountRetries+1 次：重试耗尽，应切换账号
		action := fs.HandleFailoverError(context.Background(), mock, 100, "openai", err)
		require.Equal(t, runtimefailover.Continue, action)
		require.Equal(t, 1, fs.SwitchCount)
		require.Contains(t, fs.FailedAccountIDs, int64(100))
	...
	t.Run("不同账号独立跟踪重试次数", func(t *testing.T) {
		mock := &mockTempUnscheduler{}
		fs := runtimefailover.NewState(5, false)
		err := newTestFailoverErr(400, true, false)
	...
	t.Run("重试耗尽后再次遇到同账号_直接切换", func(t *testing.T) {
		...
		// 第 testMaxSameAccountRetries+1 次: 重试耗尽 → 切换
		action := fs.HandleFailoverError(context.Background(), mock, 100, "openai", err)
		require.Equal(t, runtimefailover.Continue, action)

		// 再次遇到账号 100，计数仍为 testMaxSameAccountRetries，条件不满足 → 直接切换
		action = fs.HandleFailoverError(context.Background(), mock, 100, "openai", err)
		require.Equal(t, runtimefailover.Continue, action)
...
func TestHandleFailoverError_ContextCanceled(t *testing.T) {
	t.Run("同账号重试sleep期间context取消", func(t *testing.T) {
		...
		require.Equal(t, runtimefailover.Canceled, action)
...
func TestHandleFailoverError_IntegrationScenario(t *testing.T) {
	t.Run("模拟完整failover流程_多账号混合重试与切换", func(t *testing.T) {
		...
		// 1. 账号 100 遇到可重试错误，同账号重试 testMaxSameAccountRetries 次
	...
	t.Run("ForceCacheBilling通过错误标志设置", func(t *testing.T) {
		mock := &mockTempUnscheduler{}
		fs := runtimefailover.NewState(3, false) // hasBoundSession=false
