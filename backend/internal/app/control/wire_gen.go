...
func initialize(buildInfo BuildInfo) (*Application, error) {
	...
	controlAuthService, err := service.NewControlAuthService(client, configConfig, userRepository, controlAuthRepository, sessionSnapshotCache, settingService, turnstileService, totpService, emailService, promoCodeRepository, redeemCodeRepository, subscriptionService)
	if err != nil {
		return nil, err
	}
	...
	authHandler := handler.NewAuthHandler(configConfig, controlAuthService, controlAuthService, controlAuthService, controlAuthService, totpService, userService, settingService, serviceBuildInfo)
	...
	antigravityConnectionTester := service.NewAntigravityConnectionTester(antigravityTokenProvider, httpUpstream)
	accountTestService := service.NewAccountTestService(accountRepository, geminiTokenProvider, antigravityConnectionTester, httpUpstream, configConfig, tlsFingerprintProfileService)
	...
	jwtAuthMiddleware := middleware.NewJWTAuthMiddleware(controlAuthService)
	adminAuthMiddleware := middleware.NewAdminAuthMiddleware(controlAuthService)
