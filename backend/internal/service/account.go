...
// IsOpenAIResponsesWebSocketV2Enabled 返回 OpenAI 账号是否支持 WSv2。
// 现已标准化为 OpenAI OAuth / API Key 账号默认支持；不再读取账号级模式/开关。
func (a *Account) IsOpenAIResponsesWebSocketV2Enabled() bool {
	return a != nil && a.IsOpenAI() && (a.IsOpenAIOAuth() || a.IsOpenAIApiKey())
}
