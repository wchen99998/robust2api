...
func (s *SettingService) GetPublicSettings(ctx context.Context) (*PublicSettings, error) {
	...
	return &PublicSettings{
		...
		SiteName:                         s.getStringOrDefault(settings, SettingKeySiteName, "robust2api"),
...
func (s *SettingService) buildGatewayHotPathSnapshot(values map[string]string) *gatewayHotPathSettingsSnapshot {
	...
	snapshot.cchSigning = values[SettingKeyEnableCCHSigning] == "true"
...
func (s *SettingService) storeGatewayHotPathSnapshotFromSystemSettings(settings *SystemSettings) {
	...
	snapshot.cchSigning = settings.EnableCCHSigning
...
func (s *SettingService) GetGatewayForwardingSettings(ctx context.Context) (fingerprintUnification, metadataPassthrough, cchSigning bool) {
	if snapshot, ok := s.currentGatewayHotPathSnapshot(); ok {
		return snapshot.fingerprintUnification, snapshot.metadataPassthrough, snapshot.cchSigning
	}
