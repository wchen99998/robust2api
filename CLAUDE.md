...
├── monitoring.yaml        # Kustomization: monitoring (depends on infra)
├── apps.yaml              # Kustomization: apps (depends on monitoring)
├── infrastructure/        # ingress-nginx, cert-manager, external-dns, ClusterIssuer
├── monitoring/            # LGTM stack (Prometheus, Grafana, Tempo, Loki, Alloy)
