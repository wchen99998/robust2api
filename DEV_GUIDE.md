...
| 项目 | 说明 |
|------|------|
| **上游仓库** | wchen99998/robust2api |
| **Fork 仓库** | bayma888/robust2api-bmai |
...
| 数据库凭据 | user=`robust2api`, password=`robust2api`, dbname=`robust2api` |
...
| **release.yml** | tag `v*`, workflow_dispatch | 原生 GitHub Actions 并行构建多架构 GHCR 镜像 + GitHub Release |
...
psql -U robust2api -h 127.0.0.1 -d robust2api -f temp.sql
```
...
3. 无密码登录并重置
   ...
   ALTER USER robust2api WITH PASSWORD 'robust2api';
...
```bash
# 连接数据库
psql -U robust2api -h 127.0.0.1 -d robust2api
...
# 执行 SQL 文件
psql -U robust2api -h 127.0.0.1 -d robust2api -f migration.sql
```
...
- [上游仓库](https://github.com/wchen99998/robust2api)
