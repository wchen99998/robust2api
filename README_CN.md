...
pnpm install
pnpm run build
# 构建产物输出到 ../frontend/dist/，生产环境由 nginx sidecar 提供
...
# 7. 首次执行 bootstrap（迁移 + 可选管理员初始化）
./sub2api-bootstrap

# 8. 启动服务
./sub2api-gateway  # 推理代理
./sub2api-control  # 管理/认证平面
```

> **注意：** `gateway` 是推理代理，`control` 是管理/认证平面，控制面依赖独立的 nginx sidecar 来托管 SPA。它们仍然从 `/etc/sub2api/config.yaml` 加载配置，bootstrap 仅依赖环境变量。
>
> API 服务只会从 `/etc/sub2api/config.yaml` 读取 YAML。bootstrap 二进制只读取环境变量。

**`/etc/sub2api/config.yaml` 关键配置：**
...
`JWT_SECRET` 至少需要 32 字节。`TOTP_ENCRYPTION_KEY` 必须是 64 个十六进制字符（可用 `openssl rand -hex 32` 生成）。
如果数据库里已经有管理员，可以在运行 `sub2api-bootstrap` 时省略 `ADMIN_EMAIL` 和 `ADMIN_PASSWORD`。
...
`/etc/sub2api/config.yaml` 还支持以下安全相关配置：
...
# HTTP/1.1 回退
curl --http1.1 -I http://localhost:8080/readyz
...
sub2api/
├── backend/                  # Go 后端服务
│   ├── cmd/gateway/          # Gateway 推理入口
│   ├── cmd/control/          # Control 管理入口
